(function (root, factory) {
  ...
  function thai(amount) {
    ...
    var txt = (a < 0 && (baht > 0 || satang > 0)) ? 'ลบ' : '';
  ...
  function english(amount) {
    ...
    var parts = Math.abs(a).toFixed(2).split('.');
    var baht = parseInt(parts[0], 10), satang = parseInt(parts[1], 10);
    var txt = (a < 0 && (baht > 0 || satang > 0)) ? 'minus ' : '';
    if (baht > 0) txt += englishInt(baht) + ' baht';
    else if (satang === 0) txt += 'zero baht';
    if (satang > 0) txt += (baht > 0 ? ' ' : '') + englishInt(satang) + ' satang';
    return txt;
  }
